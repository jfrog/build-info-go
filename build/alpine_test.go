...
// Scenario #34: explicitly requested packages must get root-level requestedBy ([][]string{{}}).
func TestBuildRequestedBy_DirectPackages(t *testing.T) {
	...
	assert.Equal(t, [][]string{{}}, result["curl"],
		"explicitly requested package must have root-level requestedBy [[]]")
}
...
func TestBuildRequestedBy_TransitiveDeps(t *testing.T) {
	...
	// curl is directly requested → root level
	require.Contains(t, result, "curl")
	assert.Equal(t, [][]string{{}}, result["curl"])
...
func TestAlpineModule_ScopeClassification(t *testing.T) {
	...
	assert.Equal(t, [][]string{{}}, depByID["curl:8.5.0-r0"].RequestedBy,
		"explicitly requested package must have root-level requestedBy")
