...
func (config *gradleRunConfig) GetCmd() *exec.Cmd {
	...
	filteredTasks := handleGradleCommandProperties(config.tasks)
	cmd = append(cmd, filteredTasks...)
...
func handleGradleCommandProperties(tasks []string) []string {
	...
	for _, task := range tasks {
		if !strings.HasPrefix(task, systemPropertiesFlag) && !strings.HasPrefix(task, projectPropertiesFlag) {
			filteredArgs = append(filteredArgs, task)
			continue
		}
		splitProperties := strings.Split(task, "=")
		filteredArgs = append(filteredArgs, fmt.Sprintf(`%s="%s"`, splitProperties[0], splitProperties[1]))
	}
	return filteredArgs
}
