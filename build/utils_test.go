...
func validateModule(t *testing.T, module entities.Module, expectedDependencies, expectedArtifacts int, moduleName string, moduleType entities.ModuleType, depsContainChecksums bool) {
	...
	if expectedArtifacts > 0 {
		...
		assert.Equal(t, "123", module.Artifacts[0].Sha1, "Unexpected Sha1 field.")
		assert.Equal(t, "456", module.Artifacts[0].Md5, "Unexpected MD5 field.")
		assert.Equal(t, "789", module.Artifacts[0].Sha256, "Unexpected SHA256 field.")
	}
	assert.Equal(t, moduleType, module.Type)
	assert.Equal(t, depsContainChecksums, !module.Dependencies[0].IsEmpty())
	if depsContainChecksums {
		assert.NotEmpty(t, module.Dependencies[0].Sha1, "Empty Sha1 field.")
		assert.NotEmpty(t, module.Dependencies[0].Md5, "Empty MD5 field.")
		assert.NotEmpty(t, module.Dependencies[0].Sha256, "Empty SHA256 field.")
	}
}
