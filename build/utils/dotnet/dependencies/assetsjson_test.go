...
func TestAssetsDependenciesRetainedWithoutCachedPackage(t *testing.T) {
	...
	require.Contains(t, allDependencies, "missing.package")
	assert.Equal(t, "Missing.Package:1.2.3", allDependencies["missing.package"].Id)
	assert.Empty(t, allDependencies["missing.package"].Checksum.Sha1)
	assert.Empty(t, allDependencies["missing.package"].Checksum.Sha256)
	assert.Empty(t, allDependencies["missing.package"].Checksum.Md5)
}
...
func TestAssetsDependenciesChecksumsAndPrivateScope(t *testing.T) {
	...
	privateDependency := allDependencies["private.package"]
	...
	publicDependency := allDependencies["public.package"]
